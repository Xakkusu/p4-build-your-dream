    ...
    if request.method == "POST":
        ...
        if future_content_request_form.is_valid():
            ...
            messages.add_message(
                ...
                "We recceived your request, after it is approved and it"
                + " is deemed to be suitable for us, it will be shown here!"
            )
    future_content_request = FutureContentRequest.objects.filter(
        status_of_request=2).order_by("created_on")
    future_content_request_form = FutureContentRequestForm()
    future_content_request_count = future_content_request.filter(
        status_of_request=2).count()

    paginate_by = 4

    paginator = Paginator(FutureContentRequest.objects.filter(
        status_of_request=2).order_by("created_on"), 4)
