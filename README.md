...
- [FEATURES](#features)
    ...
    - [Show Build Page](#show-build-post)
    - [Create Build Post](#create-a-build-post-page)
    - [Search build via tags](#search-build-post-via-tags)
    - [Future Content](#future-content-page)
...
### Fixed Bugs
1. I had a lot of problems with the deployment on Heroku and the way my settings and env.py file were set up:
    ...
    > The second time I had many fix commits as my static files were not accepted when uploading. With help from the tutor team we figured out that a minor spelling mistake in my env.py file was the reason for not reading my secret data in properly. 
    
    > My cloudinary files where somehow not able to be deployed on Heroku. After a lot of tries (and way too many commits) I changed the way it was set-up in my env.py as well in settings.py as the API and key was correctly in Heroku, so the problem lied in accessing everything correctly in those files, mainly in my env.py file.

2. My html templates were not displayed at all due to the way they were set up in the settings.py file. I had to set up my templates's DIRS differently in settings.py the way they are now. 

3. At first I used another way for my class based view to Create Build Post which did not save my data from the create build post form to my database. I decided to start anew with the class and diffferent source material. I used the following tutorial as other students recommended me [Dee Mc](https://www.youtube.com/watch?v=vXMTp_1_L7Y&list=PLXuTq6OsqZjbCSfiLNb2f1FOs8viArjWy&index=10)'s playlist and especially her videos about creating, editing and deleting posts. In my previous attempt I used the wrong template name, did not define the model I was using as well as returning the form in the wrong way which was resolved by following her tutorials.

4. My form/view for future content requests did not save the form data to the database. I rewrote my class based view in a function based view as I wanted to start anew again, instead of creating a method within my class based view. Rewriting my code I deffined the returned data minorly differently, but understood every step better. In the end my code itself wasn't that much different, just the outside "package" was better and solved my problem with of defining variables and returning the correct values.

5. The page for deleting the build post was not wired up and throwing an error. I added an url path as well as adding a migrate I forgot after changing my models. Moreover, I added the correct template_name to my DeleteBuildClass & displayed the delete button by calling the correct build post id. 

6. When implementing the tags via these two ([1](https://www.youtube.com/watch?v=213swbH8j_o), [2](https://www.youtube.com/watch?v=Wy3yrZ-bbvE&t=41s)) videos I encountered a few problems with my builds views.py as I did not return my tags correctly in my already written code. In order to use the tags on the index page I rewrote my build post list into a function based view and adapted the tutorial I used to my already existing code returning the correct values in it, inlcuding the tags. 

7. I forgot to wire up my searchbuildtags view and was not able to see the build posts in my search page. I added an url path for it and used the correct attribute in my html template to get access to the build post slugs ``` x-bind:href="build.slug ```

8. When I selected a build post from my search page I got an error as the build post was not correctly selected due to the url path. I added "search/" in front of all the pages in urls.py to wire them up correctly and still be able to see my search page. This way I was able to access build posts from my landing and search page.

9. The way pagination was taught in our classes didn't really work with all additional functionality features for my project and was displayed wrong (didn't limit post number and did not add second page) on the index page. After some research I found this [post](https://stackoverflow.com/questions/74615456/how-to-add-pagination-in-django) on Stackoverflow to solve this problem and limit the amount of posts shown to 8. I later adjusted all other pagination features to the way it was done in the post.

10. When editing a comment it created a new one as the JavaScript code had a mistake in it and did not change  the submit comment button to an update one and thus created a new comment instead of editing the old one. This was easily fixed, by changing the JavaScript code, however I did this in my staticfiles folder first and hence had to correct the same mistake twice for my static folder as well to focus the correct element and change it to upload. Moreover, I implemented an approve field to the comment model as the comment model did not work as intended with the status field. This also made it possible to show if a comment is waiting for approval.

11. Using alpine.js for the first time caused quite some issues with the validators. All other validator errors that I corrected, I did not categorize as a bug, yet the alpine.js problems should be mentioned under bugs due to its severity. Even though the code worked on the surface W3C has problems with some markup languages such as alpine.js. As many developers before me had the same problems they came up with a way to add prefixes that are recognized by W3C. I read through the following [github discussion](https://github.com/alpinejs/alpine/discussions/3397) by icaliman and used the solution there to pass the W3C validator while using alpine.js.
